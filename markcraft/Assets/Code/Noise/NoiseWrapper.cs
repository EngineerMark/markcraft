...
public class NoiseWrapper
{
    ...
    public static float Perlin3D(float x, float y, float z)
    {
        ...
        float ca = Mathf.PerlinNoise(x, x);

        float abc = ab + bc + ac + ba + cb + ca;
        return abc / 6f;
    }

    public static float Multifractal2(Vector3 pos)
    {
        return Multifractal2(pos.x, pos.z);
    }

    public static float Multifractal2(float x, float y)
    {
        x = Mathf.Abs(x);
        y = Mathf.Abs(y);
        ...
        float freq = 0.5f;
        float signal = SimplexNoise.Noise.Generate((float)x * 0.25f, (float)y * 0.25f);
        float offset = 0.0f;
        ...
        result += 0.2f * SimplexNoise.Noise.Generate((float)x * 0.25f, (float)y * 0.25f);
        ...
        for (int i = 1; i < 4; i++)
        {
            ...
            signal = SimplexNoise.Noise.Generate(x, y);
            signal = offset - signal;
            signal *= signal;
        ...
        result = (float)Math.Tanh(result * 1.2f);
        return result * 0.5f;
    }
